...
package com.app.browser_components.service.fxa
 ...
 */
class FirefoxAccount internal constructor(private val inner: InternalFxAcct) :
    ...
    override fun getAccessToken(singleScope: String): Deferred<AccessTokenInfo> {
        return scope.async {
            inner.getAccessToken(singleScope).let {
                AccessTokenInfo.fromInternal(
                    it
                )
            }
        }
