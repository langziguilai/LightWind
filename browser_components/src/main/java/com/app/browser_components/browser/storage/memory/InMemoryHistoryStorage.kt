...
package com.app.browser_components.browser.storage.memory
...
/**
 * An in-memory implementation of [com.app.browser_components.concept.storage.HistoryStorage].
 */
    ...
    override fun getAutocompleteSuggestion(query: String): HistoryAutocompleteResult? = synchronized(pages) {
        segmentAwareDomainMatch(query, pages.keys)?.let { urlMatch ->
            return HistoryAutocompleteResult(
                query, urlMatch.matchedSegment, urlMatch.url,
                AUTOCOMPLETE_SOURCE_NAME, pages.size)
        }
    }
