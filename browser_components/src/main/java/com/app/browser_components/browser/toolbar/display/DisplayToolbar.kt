...
package com.app.browser_components.browser.toolbar.display
...
import com.app.browser_components.concept.toolbar.Toolbar.SiteSecurity
...
internal class DisplayToolbar(
    ...
    internal val siteSecurityIconView = AppCompatImageView(context).apply {
        ...
        setImageResource(R.drawable.mozac_ic_globe)
    ...
    fun setSiteSecurity(secure: SiteSecurity) {
        val (image, color) = when (secure) {
            SiteSecurity.INSECURE -> Pair(R.drawable.mozac_ic_globe, securityIconColor.first)
            SiteSecurity.SECURE -> Pair(R.drawable.mozac_ic_lock, securityIconColor.second)
        }
